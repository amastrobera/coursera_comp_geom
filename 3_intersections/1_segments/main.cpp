...
int main() {
    ...
    double limit_coord = 10000;

    string segStr1;
    getline(cin, segStr1);
    istringstream segStream1(segStr1); 

    crs::Segment s1;
    string strCoord1;
    
    getline(segStream1, strCoord1, ' ');
    istringstream(strCoord1) >> s1.p1.x;
    getline(segStream1, strCoord1, ' ');
    istringstream(strCoord1) >> s1.p1.y;
    getline(segStream1, strCoord1, ' ');
    istringstream(strCoord1) >> s1.p2.x;
    getline(segStream1, strCoord1, ' ');
    istringstream(strCoord1) >> s1.p2.y;
    if (abs(s1.p1.x) > limit_coord || 
        abs(s1.p1.y) > limit_coord ||
        abs(s1.p2.x) > limit_coord ||
        abs(s1.p2.y) > limit_coord ) {
        cerr << "bad input coordinates: " << strCoord1 
    ...
    string segStr2;
    getline(cin, segStr2);
    istringstream segStream2(segStr2); 
    ...
    getline(segStream2, strCoord2, ' ');
    istringstream(strCoord2) >> s2.p2.x;
    getline(segStream2, strCoord2, ' ');
    istringstream(strCoord2) >> s2.p2.y;
    if (abs(s2.p1.x) > limit_coord || 
        abs(s2.p1.y) > limit_coord ||
        abs(s2.p2.x) > limit_coord ||
        abs(s2.p2.y) > limit_coord ) {
        cerr << "bad input coordinates: " << strCoord2 
             << " (coordinate > " << limit_coord << ")" << endl;
        exit(1);
    }
