...
int main() {
    ...
    double limit_coord = 1000;

    crs::Polygon poly1, poly2;
    
    {
        ...
        if (n < 3 || n > 100)  {
            cerr << "bad input coordinates: n (" << n 
                 << ") not in range [3,100]" << endl;
            exit(1);
        }
        
        string polyString;
        getline(cin, polyString);
        istringstream polyStream(polyString);
        ...
        while (n--) {
            ...
            string coordString;
            getline(polyStream, coordString, ' ');
            istringstream(coordString) >> p.x;
            getline(polyStream, coordString, ' ');
            istringstream(coordString) >> p.y;
            
            if (abs(p.x) > limit_coord || abs(p.y) > limit_coord ) {
                cerr << "bad input coordinates: " << coordString 
                     << " (|coordinate| > " << limit_coord << ")" << endl;
                exit(1);
            }
            
            poly1.vertices.push_back(p);
        }
